...
public class KdTree {
	...
	public void insert(Point2D p) {
		...
		if (contains(p))
			return; // do not insert duplicate point
		...
		if (isEmpty()) {
			...
			if (isLeft) {
				if (parent.isVertical) {
					newNode.rect = new RectHV(parent.rect.xmin(), parent.rect.ymin(), parent.point.x(),
							parent.rect.ymax());
				} else {
					newNode.rect = new RectHV(parent.rect.xmin(), parent.rect.ymin(), parent.rect.xmax(),
							parent.point.y());
				}
				parent.left = newNode;
			} else {
				if (parent.isVertical) {
					newNode.rect = new RectHV(parent.point.x(), parent.rect.ymin(), parent.rect.xmax(),
							parent.rect.ymax());
				} else {
					newNode.rect = new RectHV(parent.rect.xmin(), parent.point.y(), parent.rect.xmax(),
							parent.rect.ymax());
				}
	...
	private void drawAux(Node n, double x0, double y0, double x1, double y1) {
		...
		drawPoint(n.point);
		if (n.left != null)
			drawAux(n.left, x0, y0, n.point.x(), y1);
		if (n.right != null)
			drawAux(n.right, n.point.x(), y0, x1, y1);
	}
	...
	private void rangeAux(Node n, RectHV rect, Queue<Point2D> queue) {
		...
		if (n.rect.intersects(rect)) {
			...
			if (n.left != null)
				rangeAux(n.left, rect, queue);
			if (n.right != null)
				rangeAux(n.right, rect, queue);
		} else {
			return;
		}
	}
